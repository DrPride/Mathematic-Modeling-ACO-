...
const double BETA = 3.0;  //期望因子，节点间距离的重要程度
const double ROU = 0.5; //信息素残留参数
//0 3 5 1 2 4 6 7
const int N_ANT_COUNT = 100; //蚂蚁数量
const int N_IT_COUNT = 3000; //迭代次数
const int N_CITY_COUNT = 8; //节点数量
...
public:
        ...
        int m_nAllowedCity[N_CITY_COUNT]; //没去过的节点
        int m_nCurCityNo; //当前所在节点编号
        int m_nMovedCityCount; //已经去过的节点数量

public:

        int ChooseNextCity(); //选择下一个节点
        void Init();//初始化
        void Move();//蚂蚁在节点间移动
        void Search(); //搜索路径
...
void CAnt::Init()
{
        ...
        {
                m_nAllowedCity[i] = 1; //设置全部节点为没有去过
...
int CAnt::ChooseNextCity()
{
        ...
        double dbTotal=0.0;
        double prob[N_CITY_COUNT]; //保存各个节点被选中的概率

        for (int i = 0; i < N_CITY_COUNT;i++)
        {
                if (m_nAllowedCity[i] == 1) //节点没去过
                {
...
void CAnt::Move()
{
        int nCityNo=ChooseNextCity(); //选择下一个节点

        m_nPath[m_nMovedCityCount]=nCityNo; //保存蚂蚁走的路径
        m_nAllowedCity[nCityNo]=0;//把这个节点设置成已经去过了
        m_nCurCityNo=nCityNo; //改变当前所在节点为选择的节点
        m_nMovedCityCount++; //已经去过的节点数量加1
}
...
void CTsp::Search()
{
        ...
        for(int i = 0; i < N_IT_COUNT; i++)
        {
                ...
                for (int j = 0; j < N_ANT_COUNT; j++)
                {
                        if (m_cAntAry[j].m_dbPathLength < bestLength )
                        {
