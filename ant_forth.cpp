...
//0 3 5 1 2 4 6 7
const int N_ANT_COUNT = 100; //蚂蚁数量
...
int mutex[N_ANT_COUNT] = {0};//互斥量数组生成
        //即同一个地点不能进入，先设置为均可进入
...
const double SPEED_VARIANCE = 1;//速度随机分布的方差
...
int xx = 0;   //这里定义x一个全局变量并且初始付值0，这个的功用将会在子函数UNIFORM中得以体现

void UNIFORM(double *p)
{
	int i, a;
	double f;
	for (i = 0; i<2; i++, xx = xx + 689)
	{
		a = rand() + xx;  //加上689是因为系统产生随机数的更换频率远远不及程序调用函数的时间
		a = a%1000;
		f = (double)a;
		f = f / 1000.0;
		*p = f;
...
//这个函数产生的正态分布随机数更为neat
//用于产生速度
double Speedproducer(double lowwer, double upper, double D)//D为方差
{
	int i, j;
	double A, B, C, E, r;
	double uni[2];
	...
	srand((unsigned)time(NULL));  //随机数种子采用系统时钟
        ...
        B = 2 * PI*uni[1];
        C = A*cos(B);
        E = (lowwer + upper) / 2;//数学期望为均值，换言之，其均值可以调控
        r = E + C * D;    //E,D分别是期望和方差
        if(r >= lowwer && r <= upper) return r;
        else return BoxMuller(lowwer, upper, D);
}

double speed()
{
        return Speedproducer(1, 3, SPEED_VARIANCE)
        //返回符合期望的时间
}
...
public:
        ...
        double g_Wait[N_CITY_COUNT];//记录等待的时间
...
int CAnt::ChooseNextCity()
{
        ...
        for (int i = 0; i < N_CITY_COUNT;i++)
        {
                if(mutex[i] == 1)
                {
                        ...
                        if(rnd(0.35, 1.0) > 0.5)//这个选择应该为低概率；
                        {
                                Then_Wait(i, WaitTime);//根据0-1模型做选择
                                return nSelectedCity = i;
                                return
                        }
                        else continue;//放弃进入
                }
                else mutex[i] == 1; //判断是否可以进入
                temp_enteringTime[i] = m_nTimeCost[m_nCurCityNo] + g_Walk[m_nCurCityNo][i] + TimeHereCost + g_Wait[m_nCurCityNo];
...
//等待函数
void CAnt::Then_Wait(int nCityNo, double waitTime)
{
        g_Wait[nCityNo] += WaitTime;//此处应该有线程通信，获取别的线程里的对象的值
}
//计算蚂蚁走过的路径长度
...
public:
        CAnt m_cAntAry[N_ANT_COUNT][3]; //蚂蚁数组
        CAnt* m_cBestAnt; //定义一个蚂蚁变量，用来保存搜索过程中的最优结果
                        //该蚂蚁不参与搜索，只是用来保存最优结果
        //static pthread_mutex_t spot_mutex[N_CITY_COUNT];
...
void CTsp::Search()
{
        ...
        for(int i = 0; i < N_IT_COUNT; i++)
        {
                ...
                {
                        err1 = phread_create(&tid1, NULL, m_cAntAry[j][0].Search, NULL);
                        err2 = phread_create(&tid2, NULL, m_cAntAry[j][1].Search, NULL);
                        err3 = phread_create(&tid3, NULL, m_cAntAry[j][2].Search, NULL);
                         if(err1 != 0)
                        {
                ...
                for (int j = 0; j < N_ANT_COUNT; j++)
                {
                        ...
                        int m_dWholeTime = (330-(m_cAntAry[j][0].m_dbPathLength)*30) +(330-(m_cAntAry[j][1].m_dbPathLength)*30) + (330-(m_cAntAry[j][2].m_dbPathLength)*30);
                        int m_dBestTime = (330-(m_BestAnt[j][0].m_dbPathLength)*30) + (330-(m_BestAnt[j][1].m_dbPathLength)*30) + (330-(m_BestAnt[j][2].m_dbPathLength)*30);
                        if (m_dWholeTime > m_dBestTime)
                        {
                                m_cBestAnt = &m_cAntAry[j];
                                //指向头地址
                                for(int k = 0; k<N_CITY_COUNT;k++)
                                {
                                        sprintf(cBuf, "%d ", m_cAntAry[j][0].m_nPath[k]);
                                        sprintf(dBuf, "%d ", m_cAntAry[j][1].m_nPath[k]);
                                        sprintf(eBuf, "%d ", m_cAntAry[j][0].m_nPath[k]);
                                        printf(cBuf);
                                        printf(dBuf);
                                        printf(eBuf);
                                }
...
DWORD WINAPI TspThread(LPVOID lpParameter)
{
...
{
        HANDLE Thread1;
        Thread1 = CreateThread(NULL, 0, TspThread, NULL, 0, NULL);
        //此处为单线程
        //为改为多线程对返回数据进行再次清洗提供方便
        CloseHandle(Thread1);
